  ...
  return (
    <li className="formToDo__CartsElement" key={id}>
      ...
      <div className="formToDo__RowButtons">
        {buttonArr.map(el => <Button key={el.name} onClick={e => buttonFunction(e, id)} className="rowButton" variant={el.variant} color={el.color} endIcon={el.icon}>{el.name}</Button>)}
      </div>
    </li>
  );
};
