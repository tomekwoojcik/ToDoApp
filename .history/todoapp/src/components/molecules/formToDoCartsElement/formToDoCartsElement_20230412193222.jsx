...
const FormToDoCartsElement = (arrayElement) => {
  ...
  const buttonArr = [
    { name: "Update", buttonFunction: buttonFunction(e, id), variant: "contained", color: "primary", icon: <UpgradeIcon className="buttonIcon" style={{ fontSize: "small" }} /> },
    { name: "Send", variant: "contained", color: "primary", icon: <UpgradeIcon className="buttonIcon" style={{ fontSize: "small" }} /> },
    { name: "Delete", variant: "contained", color: "primary", icon: <UpgradeIcon className="buttonIcon" style={{ fontSize: "small" }} /> }]
  return (
    <li className="formToDo__CartsElement" key={id}>
      {!update ? title : FormToDoUpdateRow}
      <div className="formToDo__RowButtons">
        {buttonArr.map((el) => { return (<Button key={el.name} onClick={e => buttonFunction} className="rowButton" variant={el.variant} color={el.color} endIcon={el.icon}>{el.name}</Button>)})}
