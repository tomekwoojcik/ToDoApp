...
export const FormProvider = ({ children }) => {
  ...
  const buttonArr = [
    {
      ...
      color: "primary",
      icon: (
        <UpgradeIcon className="buttonIcon"/>
      ),
    },
    {
      name: "READY",
      variant: "contained",
      color: "success",
      icon: <SendIcon className="buttonIcon"/>,
    },
    {
      name: "Delete",
      variant: "contained",
  ...
  const handleUpdate = () => {
    ...
    data.title = getValue;
    deleteData(data.id, "toDoElements");
    const newData = getData("toDoElements").filter(el => el.id === data.id);
    const dataElements = getData("toDoElements").filter(el => el.id !== data.id);
    dataElements.push(newData);
    sentValue("toDoElements", dataElements);
    setHandleData(dataElements);
    setUpdate(false);
  };
  ...
  const iconArr = [
    {
      icon: <InstagramIcon className="footerIcon" />,
      key: 1
    },
    {
      icon: <FacebookIcon className="footerIcon" />,
      key: 2
    },
    {
      icon: <LinkedInIcon className="footerIcon" />,
      key: 3
    },
    {
      icon: < TwitterIcon className="footerIcon" />,
      key: 4
    }
